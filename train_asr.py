...
def main():
    ...
    parser.add_argument('--tokenizer_dir', type=str, default=False,
                        help='tokenizer dir')
    ...
    tokenizer = data_asr.train_bpe(trans_train, vocab_size=args.bpe_vocab_size, output_dir=args.tokenizer_dir,
    ...
    optimizer = torch.optim.AdamW(model.connector.parameters(), lr=args.peak_lr, weight_decay=args.weight_decay)
    # print(model.config)
    ...
    if args.resume:
        ...
        model.connector.load_from_dir(os.path.join(args.output_dir, 'latest'), device)
